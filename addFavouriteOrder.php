	...
	<!-- You COULD just put both menus in the markup,
		...
		<div id="bg">
			<div id="container"><?php include 'header1.php'; ?>
				...
				<div id="addFavouriteOrder">
					<?php
						$action = isset($_GET['action'])?$_GET['action']:NULL;
						if(($action==NULL) || ($action!='saveFavOrder'))
						{
							
					?>	
							<form action="addFavouriteOrder.php" data-validate="parsley" method="get">
								<table id="cartTable">
									<tr>
										<td class="check">
										Select
							...
							<?php
								...
								$query = "SELECT * FROM `order` WHERE customer_id = '$customer_id' AND favourite_order=0";
								...
								while($row=$result->fetch_assoc())
								{
									...
									$value=1;
							?>	
									<p id="addErrorContainer"></p>
									<tr>
										<td class="check">
										<input type="checkbox" name="favOrder[<?php echo $order_id; ?>]" value="<?php echo $order_id; ?>" data-group="mygroup" data-mincheck="1" data-error-container="#addErrorContainer">
										</td>
										<td  class="date">
										<?php echo "$date_time"; ?>
										</td>
							...
							?>
								...
								<input type="hidden" name="action" value="saveFavOrder">
							</form>
